...
type Socket = Connected | Errored | { status: "ClosedSocket" }
...
type ConnectButtonClicked = { type: "ConnectButtonClicked", handler: (_: Action) => void }
...
type Action = ReceivedPayload | InputUpdated | ConnectButtonClicked | DisconnectButtonClicked

/* UPDATE */

const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    case "InputUpdated": return { ...state, url: action.value }
    ...
    case "PayloadReceived":
      switch (action.value.isOk) {
        case true: return { ...state, controller: action.value.value }
...
function App() {
  ...
  return (
    <form action="">
      ...
      />
      <button type="button" onClick={_ => dispatch({ type: "ConnectButtonClicked", handler: dispatch })} >Connect</button>
      <button type="button" onClick={_ => dispatch({ type: "DisconnectButtonClicked" })}>Disconnect</button>
    </form>
  );
}

const tryCreateSocket = (url: string, dispatch: (_: Action) => void): Socket => {
  try {
    var connection = new WebSocket(url);
    connection.onmessage = function (e: MessageEvent<string>) {
      dispatch({ type: "PayloadReceived", value: C.parseJSON(JSON.parse(e.data)) })
      console.log(e.data, typeof (e.data));
    }
    return { status: "Connected", websocket: connection }

  } catch (error) {
    return { status: "SocketError", error: "Could not connect to socket: " + error }
  }
}
